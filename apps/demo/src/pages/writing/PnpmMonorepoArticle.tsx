...
of the body of a function component.`}</CodeBlock>
            ...
            <P>
              ...
              Query saying "the provider exists, but I can't see it" — because it's looking in a
            ...
            <P>
              ...
              package's <Code>devDependencies</Code> can only pin one version. Whichever app doesn't
              match ends up with not just two instances — but two{" "}
...
module.exports = function getLodashVersion() {
  ...
  "devDependencies": { "lodash": "4.17.20" }
            ...
            <P>
              ...
              <Code>dist/</Code> doesn't exist yet. You have to manually build every shared package
              before any consumer can import from it. And if packages depend on each other, you have
              to build them in the right order — leaf packages first, then their consumers, up the
              chain.
            </P>
  ...
  "prepare": "pnpm build"
            ...
            <P>
              ...
              <Code>dist/</Code> created at install time. When <Code>tsc</Code> rebuilds, it writes
              new files with new inodes — but the consumer's hard links still point to the old ones.
              The consumer doesn't see the new build.
            </P>

            <P>
              This is where <A href="https://github.com/tiktok/pnpm-sync">pnpm-sync</A> (by TikTok)
              comes in. It re-copies <Code>dist/</Code> into each consumer's{" "}
            ...
            <P>
              We wire these into the existing scripts. The <Code>prepare</Code> hook now also writes
              the sync config, and <Code>postbuild</Code> runs the copy after every build:
            </P>
  ...
  "prepare": "pnpm sync:prepare && pnpm build"
            ...
            <P>
              On a fresh clone: <Code>pnpm install</Code> → <Code>prepare</Code> runs → writes sync
              config → builds <Code>dist/</Code> → <Code>postbuild</Code> fires →{" "}
            ...
            <P>
              ...
              recompilation too. <Code>tsc --watch</Code> doesn't support running a command on
              success, so we swap it for{" "}
  ...
  "devDependencies": {
    ...
    "typescript": "^5"
            ...
            <P>
              ...
              <Code>transpilePackages</Code> in Next.js or similar bundler-level source resolution,
  ...
  "include": ["src/**/*.ts", "src/**/*.tsx"]
            ...
            <P>
              ...
              strict dependency isolation. But even with isolated installs, workspace packages are
              still <Strong>symlinked to their source directory</Strong>. There's no{" "}

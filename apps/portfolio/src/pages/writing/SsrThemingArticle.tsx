...
export function SsrThemingArticle() {
  return (
    <Article.Layout slug="ssr-theming" writtenWithAI tocItems={tocItems}>
      ...
      <P>
        Implementing dark mode often seems like a trivial feature. In practice, it turns out to be
        surprisingly tricky in server-rendered applications.
      </P>
      <P>
        In a purely client-side app, persisting the theme preference is straightforward: store the
        value somewhere in the browser like <Code>localStorage</Code> and read it again when the app
        loads. However, doing the same in an SSR app comes with a catch.
      </P>

      <H3 id="storing-it-in-local-storage">Storing it in localStorage</H3>

      <P>
        Naturally, the first step is to reach for the same approach that works in a client-side app:
        persist the theme in <Code>localStorage</Code>.
      </P>
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <ThemeContext.Provider value={{ theme, toggleTheme }}>
      ...
      <P>
        There's a problem though — the server has no <Code>localStorage</Code>, so this throws an
        error immediately.
      </P>
      ...
      <P>
        ...
        <Code>typeof window !== &quot;undefined&quot;</Code>. This way, the server falls back to the
        default theme and only the browser reads from <Code>localStorage</Code>:
      </P>
  ...
  return (
    <ThemeContext.Provider value={{ theme, toggleTheme }}>
      ...
      <P>
        ...
        That happens because the server sends the initial HTML before any JavaScript runs — it has
        no access to <Code>localStorage</Code>, so it always renders the default theme. By the time
        the client loads and reads your stored preference, the user has already seen the wrong one.
      </P>

      <P>
        To fix this, the server needs to know the theme before it sends the initial HTML. Since the
        server can't read localStorage, we need to store it somewhere it can read — cookies.
      </P>
...
export function useTheme(): ThemeContextValue {
  ...
  // ...
      ...
      <P>
        The server reads the cookie, sets the correct class on <Code>{"<html>"}</Code>, and the
        first paint matches the user's preference. No flash. (Later we'll add a client cache so
        client-side navigations don't refetch the theme; this is the minimal version.)
      </P>

      <P>
        Try it: toggle the theme and refresh the page. The correct theme appears on first load with
        no flash.
      </P>
      ...
      <P>
        If you tried toggling in the demo above, you likely noticed that it's not instant. There's a
        delay between when you click the toggle and when the theme changes. That's because every
        toggle has to wait for two server round-trips before the UI updates.
      </P>
      ...
      <P>
        To make the toggle feel instant, we can optimistically apply the new theme before the server
        responds.
      </P>
...
export function useTheme(): ThemeContextValue {
  ...
  return ctx;
      ...
      <P>
        ...
        invalidation happen in the background. If the server call fails, the optimistic value rolls
        back to <Code>serverTheme</Code> automatically. The <Code>requestRef</Code> guard ensures we
        only call <Code>router.invalidate()</Code> if this request is still the latest — so rapid
        toggles don't let an older response overwrite the theme. The toggle is instant now.
      </P>
      ...
      <P>
        ...
        navigation takes about a second. The app isn't re-rendering on the server — it's waiting for
        a single string.
      </P>

      <P>
        Here's why. In TanStack Start, <Code>beforeLoad</Code> runs on the <Strong>server</Strong>{" "}
        for the initial request and on the <Strong>client</Strong> for every subsequent navigation.
        When it runs on the client, <Code>getThemeServerFn()</Code> is still a server function — it
        makes a network request back to the server. The route can't finish loading until that
        request resolves. Every link click, every back button, every forward navigation pays this
        cost.
      </P>

      <P>
        Think of it like hydration. On the initial page load, the server sends the theme with the
        HTML — the client needs that to render correctly. But after hydration, the client already
        knows the theme. It's right there in memory. There's no reason to ask the server for it
        again. After the first load, this should behave like a SPA.
      </P>
      ...
      <P>
        ...
        variable; have <Code>beforeLoad</Code> check <Code>typeof window</Code>: on the server, call
        the server function; on the client, return the cached value. No network request.
      </P>
...
export function useTheme(): ThemeContextValue {
  ...
  return ctx;
      ...
      <P>
        The root route's <Code>beforeLoad</Code> calls the server on the initial request and the
        cache on the client:
      </P>
  ...
  return { theme: getThemeForClientNav() };
      ...
      <P>
        ...
        <Code>getThemeForClientNav()</Code> instead of the server, so navigation is instant. Click
      ...
      <P>Two files. The theme module with server functions, client cache, and React context:</P>

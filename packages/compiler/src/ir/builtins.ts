 ...
 *
 * Three orthogonal axes, each corresponding to a real question
 * optimization passes ask:
 *
 *   - `hasSideEffects` — can this call be *deleted* when its result
 *     is unused?  DCE asks this. `Math.random()` is `false` here:
 *     the call has no observable effect so it's safe to drop.
 *     `console.log(x)` is `true`.
 *
 *   - `isDeterministic` — do equal inputs always produce equal
 *     outputs?  CSE asks this (merge duplicate calls). LICM asks
 *     this (hoist loop-invariant calls). `Math.random()` is `false`.
 *     Pure math functions are `true`.
 *
 *   - `evaluate` — can this call be *folded* at compile time when
 *     all args are constants?  Constant propagation asks this.
 *     Only set when the builtin is both pure and deterministic AND
 *     cheap to evaluate — `JSON.parse` with a huge string arg is
 *     pure+deterministic but intentionally not folded.
 *
 * A builtin can be pure (`hasSideEffects: false`) without being
 * foldable. `Math.random()` and `Date.now()` are the canonical
 * examples — they're safe to delete if unused, but CP cannot
 * produce a constant value for them.
 */
export interface BuiltinSpec {
  ...
  /**
   * Observable side effects: throws (not counting arg-type-mismatch),
   * I/O, mutates globals, writes to arg objects.
   */
  readonly hasSideEffects: boolean;

  /** Same inputs always produce the same output (no clock / entropy). */
...
function pureDet(arity: ArityKind, evaluate: Evaluator): Omit<BuiltinSpec, "name"> {
  return { hasSideEffects: false, isDeterministic: true, arity, evaluate };
}
...
define(["Math.random"], { hasSideEffects: false, isDeterministic: false, arity: exact(0) });
define(["Date.now"], { hasSideEffects: false, isDeterministic: false, arity: exact(0) });
...
/** Is this builtin safe to delete when its result is unused? (DCE predicate.) */
export function isPureBuiltin(spec: BuiltinSpec | undefined): boolean {
  return spec !== undefined && !spec.hasSideEffects;
}

/**
 * Is this builtin foldable right now given these arg counts? (CP predicate.)
 * Checks purity + determinism + arity match + presence of `evaluate`.
...
export function canFoldBuiltin(spec: BuiltinSpec | undefined, argCount: number): boolean {
  ...
  if (spec.hasSideEffects || !spec.isDeterministic) return false;

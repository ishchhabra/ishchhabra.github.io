...
| ------------------------ | --------------- | ------------------ | -------------------------------------------------------------------------------------------------- |
| `getMemoryEffects(env?)` | `MemoryEffects` | unknown read/write | Heap reads/writes (already exists)                                                                 |
| `mayThrow(env?)`         | `boolean`       | `true`             | Can raise (TDZ, null deref, coercion, user throw, opaque call)                                     |
| `mayDiverge(env?)`       | `boolean`       | `true`             | Can fail to terminate (loops, opaque calls)                                                        |
| `isDeterministic`        | `boolean`       | `false`            | Same inputs → same output. False for `Date.now`, `Math.random`, mutable global loads, opaque calls |
| `isObservable(env?)`     | `boolean`       | `true`             | Externally visible beyond heap (`console.*`, DOM, `debugger`, opaque calls)                        |
...
```ts
isDCERemovable(op, env) = writes(op).length === 0 && !mayThrow(op, env) && !isObservable(op, env);

isSpeculatable(op, env) =
  writes(op).length === 0 && !mayThrow(op, env) && !mayDiverge(op, env) && !isObservable(op, env);
...
canReorderWith(a, b, env) =
  memoryDisjoint(a, b) && throwOrderPreserved(a, b) && observableOrderPreserved(a, b);
```
...
1. In `src/ir/core/Operation.ts`, add to the base class. **Defaults are maximally conservative** — every derived predicate must return `false` for an un-overridden op. An op opts _into_ optimization by overriding axes it can prove safe.
...
2. There is no pure trait escape hatch. Ops that are all-clean declare the five axes directly, so the effects model stays the single source of truth.
...
- **Phase 2 conservatism**: defaulting `mayThrow=true` for unaudited ops may regress optimizations that previously relied on optimistic purity metadata. Audit all-clean ops while doing Phase 2.
